...
@Component({
  ...
  template: `
    <form #myForm="ngForm" (ngSubmit)="onsubmit()" *ngIf="item">
      <div>
        <label for="ibsn">ISBNコード：</label><br />
        <span id="isbn">{{item.isbn}}</span>
      </div>
      <div>
        <label for="title">書名：</label><br />
        <input id="title" name="title" size="25" type="text"
          [(ngModel)]="item.title" />
      </div>
      <div>
        <label for="price">価格：</label><br />
        <input id="price" name="price" size="5" type="number"
          [(ngModel)]="item.price" />
      </div>
      <div>
        <label for="publisher">出版社：</label><br />
        <input id="publisher" name="publisher" type="text"
          [(ngModel)]="item.publisher" />
      </div>
      <div>
...
export class EditComponent {
  ...
  // サブミット時にeditedイベントの発生($eventにはBookオブジェクトを代入)
  onsubmit() {
